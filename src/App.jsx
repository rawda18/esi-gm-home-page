...
const LandingPage = () => {
  ...
  return (
    <div className="  bg-[linear-gradient(135deg,#020817_0%,#0A1128_50%,#020817_100%)] text-white font-sans selection:bg-blue-500/30 w-full ">
      {/* --- NAVBAR --- */}
      <nav className=" navbar navbar-expand-lg navbar-bg-dark border-bottom border-secondary px-4 py-3">
        <div className="container-fluid  mx-auto px-[80px] w-full flex items-center justify-between ">
          ...
          <div className="d-flex align-items-center gap-2">
            ...
            <div className="d-flex flex-column items-start" style={{frontFamily:"inter"}}>
              <span className=" text-white" >ESI-GM</span>
              <small className="text-secondary" style={{ fontSize: '10px' ,width:'200px'}}>
          ...
          <div className="hidden lg:flex items-center justify-between w-full" id="navbarContent">
            ...
            <div className="d-flex gap-2">
              <button className= "w-[90px] h-10 px-0 py-2">Login</button>
              <button className="w-[107px] h-[40px] px-[24px] py-[8px] bg-[#1E40AF] opacity-1 flex items-center justify-center text-white font-normal text-[14px] hover:bg-[#1e3a8a] transition-all active:scale-95 rounded-[12px] " style={{ fontFamily: "Inter, sans-serif" }}>Register</button>
            </div>
          </div>
        </div>
      </nav>
      ...
      <header className=" w-full mx-auto px-6 md:px-16 py-16 md:py-28 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div className="space-y-8">
          ...
          <h1 className="text-[32px] md:text-[40px]  font-inter leading-[1.1] tracking-tight">
          ...
          <p className="font-normal text-[15px] md:text-[16px] max-w-[520px] leading-[24px] tracking-normal text-[#94A3B8] mt-3" style={{fontFamily:'inter,sans-serif' }}>
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-6">
            <button className="w-[174px] h-[58px] bg-gradient-to-r from-[#1E40AF] via-[#1E3A8A] to-[#172554] shadow-xl shadow-blue-900/40 transition-all hover:scale-105 active:scale-95 rounded-[12px] px-[32px] py-[16px] gap-[8px] flex items-center justify-center font-normal text-[14.9px] leading-[24px] "style={{fontFamily:"inter,sans-serif"}}>
              Get Started <ArrowRight size={20} />
            </button>
            <button className="w-[116px] h-[58px] rounded-[12px] border-[1px] border-slate-700 px-[32px] py-[16px] bg-slate-800/40 flex items-center justify-center text-white font-normal text-[14.9px] leading-[24px] hover:bg-slate-800 transition-all active:scale-95 ">
              Sign In
            </button>
          </div>
        </div>
      ...
      <section className="w-full mx-auto px-6 md:px-16 py-1">
        <div className="text-center max-w-3xl mx-auto mb-10 space-y-2">
          <h3 className="text-4xl font-bold "style={{fontSize:'28px',fontFamily:'inter,sans-serif'}}>Powerful Features</h3>
          <p className="text-gray-400 text-lg"style={{fontSize:'15.2px'}}>Everything you need to manage your laboratory efficiently .</p>
        </div>
      ...
      <section className=" py-24  max-w-[1280px] mx-auto mb-[-30px] ">
        <div className="max-w-[1232px] mx-auto px-6 md:px-16">
          <div className="text-center mb-16">
            ...
            <p className="text-[#94A3B8] font-inter font-normal text-[18.6px] leading-[28px] text-center align-middle opacity-100 ">Simple workflow for all user types </p>
          </div>
...
const FeatureCard = ({ icon, title, desc }) => (
  <div className="w-full max-w-[450px] h-[195px] p-3 bg-[#0A1128]/40 border-[1px] border-[#1E40AF4D] rounded-[16px] group transition-all duration-300  hover:bg-[#0A1128]/60 hover:border-[#1E40AF] hover:-translate-y-1 backdrop-blur-sm flex flex-col justify-start ">
    ...
    <h3 className=" font-bold mb-0 text-white font-inter w-[339.33px] text-[17px] leading-[40px]">{title}</h3>
    <p className="text-[#94A3B8]  text-[14.9px]" style={{ fontFamily: 'Inter,sans-serif',lineHeight: '20px',width:'289px',fontSize:'12.7px'}}>{desc}</p>
  </div>
);
const StepCard = ({ number, role, steps, color }) => (
  <div className="w-[310px] h-[254px]  pt-[20px] pr-[24px]  pl-[20px] bg-[#0A1128] border-1 border-[#1E40AF] rounded-[16px] relative group transition-all duration-300   flex flex-col ">
    <div className="absolute -top-3 -left-3 w-[41px]  h-[41px] bg-[#1E40AF] rounded-full flex items-center justify-center text-[#E8EAF0] font-bold text-[17px] z-20 border-[1px] border-[#020617]">
      {number}
    </div>
    <div className="flex flex-col items-start gap-3 mb-[24px] ">
      <h3 className="font-bold text-[#E8EAF0] font-inter  font-bold text-[17px] leading-[28px] w-[339.33px] h-[28px] flex items-center mt-1 mb-[-10px] text-left">{role}</h3>
    </div>
    <ul className=" flex flex-col gap-[6px] relative z-10 items-start pl-0   ">
      {steps.map((step, idx) => (
        <li key={idx} className="flex items-start text-[#94A3B8] gap-[10px] text-[13px] leading-[22px]">
          <div className="w-[6px] h-[6px] bg-[#1E40AF] rounded-[33554400px] mt-2 shrink:0"></div>
